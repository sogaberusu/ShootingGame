...
void SkinModel::UpdateWorldMatrix(CVector3 position, CQuaternion rotation, CVector3 scale)
{
	...
	//平行移動行列を作成する。
	m_transMatrix.MakeTranslation(position);
	//回転行列を作成する。
	m_rotMatrix.MakeRotationFromQuaternion(rotation);
	//拡大行列を作成する。
	m_scaleMatrix.MakeScaling(scale);
	//ワールド行列を作成する。
	//拡大×回転×平行移動の順番で乗算するように！
	//順番を間違えたら結果が変わるよ。
	m_worldMatrix.Mul(mBias, m_scaleMatrix);
	m_worldMatrix.Mul(m_worldMatrix, m_rotMatrix);
...
void SkinModel::Draw(CMatrix viewMatrix, CMatrix projMatrix, int renderMode)
{
	...
	vsCb.mLightView = g_graphicsEngine->GetShadowMap()->GetLightViewMatrix();
